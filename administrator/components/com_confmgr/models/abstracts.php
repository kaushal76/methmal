...
/**
 * List Model for abstracts.
 *
 * @package     Confmgr
 * @subpackage  Models
 */
class ConfmgrModelAbstracts extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	public function __construct($config = array())
	{
		if (empty($config['filter_fields']))
		{
			$config['filter_fields'] = array(
				...
				'a.ordering', 'ordering','ordering', 'state', 'paper_id', 'abstract', 'keywords', 'theme', 'rev1ew_outcome', 'rev1ew_comments', 'type_of_submission'
			);
		}
	...
	protected function getListQuery()
	{
		...
		$query->select('a.*')->from('#__confmgr_abstract AS a');				
	...
	public function getAuthors()
	{
		...
		$query->select('u.id AS value, u.name AS text')
			->from('#__users AS u')
			->join('INNER', '#__confmgr_abstract AS a ON a.created_by = u.id')
			->group('u.id, u.name')
			->order('u.name');
