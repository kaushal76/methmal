...
/**
 * List Model for presentations.
 *
 * @package     Confmgr
 * @subpackage  Models
 */
class ConfmgrModelPresentations extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	public function __construct($config = array())
	{
		if (empty($config['filter_fields']))
		{
			$config['filter_fields'] = array(
				...
				'a.ordering', 'ordering','ordering', 'state', 'paper_id', 'presentation', 'type_of_submission', 'approved'
			);
		}
	...
	protected function getListQuery()
	{
		...
		$query->select('a.*')->from('#__confmgr_presentation AS a');				
	...
	public function getAuthors()
	{
		...
		$query->select('u.id AS value, u.name AS text')
			->from('#__users AS u')
			->join('INNER', '#__confmgr_presentation AS a ON a.created_by = u.id')
			->group('u.id, u.name')
			->order('u.name');
