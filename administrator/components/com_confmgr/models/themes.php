...
/**
 * List Model for themes.
 *
 * @package     Confmgr
 * @subpackage  Models
 */
class ConfmgrModelThemes extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	public function __construct($config = array())
	{
		if (empty($config['filter_fields']))
		{
			$config['filter_fields'] = array(
				...
				'a.ordering', 'ordering','ordering', 'state', 'user_id', 'title', 'description'
			);
		}
	 ...
	 * @return  void
	 */
	protected function populateState($ordering = 'user_id', $direction = 'ASC')
	{
		// Get the Application
	...
	protected function getListQuery()
	{
		...
		$query->select('a.*')->from('#__confmgr_theme AS a');				
		...
		if (!empty($search))
		{
			...
			{
				$search = $db->quote('%' . $db->escape(substr($search, strlen('user_id:')), true) . '%');
		...
		$sort = $this->getState('list.ordering', 'user_id');
	...
	public function getAuthors()
	{
		...
		$query->select('u.id AS value, u.name AS text')
			->from('#__users AS u')
			->join('INNER', '#__confmgr_theme AS a ON a.created_by = u.id')
			->group('u.id, u.name')
			->order('u.name');
