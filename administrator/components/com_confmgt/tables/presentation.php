...
class ConfmgtTablepresentation extends JTable {
    ...
    public function bind($array, $ignore = '') {
		...
		if(!empty($files['presentation'])){
			...
			if (UploadHelper::checkFile($file, 5)) // file name and file size (in MB) as parameters 
			{
				...
				if (!$extOK) {
					JError::raiseWarning( 500, JText::_( 'This file type is not allowed. Only '.$params->get('file_type_presentation').' are allowed'));
					return false;
				}
			}else{
    ...
    protected function _getAssetName() {
        $k = $this->_tbl_key;
        return 'com_confmgt.presentation.' . (int) $this->$k;
    }
