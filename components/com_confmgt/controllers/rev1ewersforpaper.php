...
class ConfmgtControllerRev1ewersforPaperform extends ConfmgtController
{
	...
	private function save_common($url_success, $url_fail)
	{
		...
		if ($data === false) {
			...
			$app->setUserState('com_confmgt.edit.revforpaperr.data', JRequest::getVar('jform'),array());

			// Redirect back to the edit screen.
			$id = (int) $app->getUserState('com_confmgt.edit.arevforpaper.id');
		...
		if ($return === false) {
			// Save the data in the session.
			$app->setUserState('com_confmgt.edit.arevforpaper.data', $data);
        ...
        $app->setUserState('com_confmgt.edit.arevforpaperr.id', null);
		...
		$app->setUserState('com_confmgt.edit.arevforpaper.data', null);
	}
